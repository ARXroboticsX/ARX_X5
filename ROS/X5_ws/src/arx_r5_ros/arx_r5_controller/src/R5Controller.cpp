...
namespace arx::r5
{
    R5Controller::R5Controller(ros::NodeHandle nh)
    {
        ...
        else if (arm_control_type == "normal_old")
        {
            ...
            joint_state_subscriber_ = nh.subscribe<arm_control::JointControl>(
                sub_topic_name, 10, &R5Controller::OldCmdCallback, this);
            // 定时器，用于发布关节信息
    ...
    void R5Controller::OldCmdCallback(const arm_control::JointControl::ConstPtr &msg)
    {
        std::vector<double> joint_positions = {0, 0, 0, 0, 0, 0};
    ...
    void R5Controller::PubOldJointState(std::vector<double> joint_pos_vector,
    ...
    void R5Controller::PubOldEndState(std::vector<double> xyzrpy, double gripper)
    {
